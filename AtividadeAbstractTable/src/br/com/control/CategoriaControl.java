...
import static br.com.view.CRUDCategoriasJFrame.*;
...
public class CategoriaControl {
    ...
    public void previousPag() {
        table.clearTable();
        cat = persistence.listar(((currentPage * rowsPerPage) - (rowsPerPage + rowsPerPage)), rowsPerPage);
    ...
    public void nextPag() {
        table.clearTable();
        cat = persistence.listar((currentPage * rowsPerPage), rowsPerPage);
        for (Categoria categoria : cat) {
            table.addRow(categoria);
        }
        currentPage++;
    ...
    public void lastPage() {
        table.clearTable();
        cat = persistence.listar((totalPage * rowsPerPage) - rowsPerPage, rowsPerPage);
        for (Categoria categoria : cat) {
            table.addRow(categoria);
        }
    ...
    public void firstPage() {
        table.clearTable();
        cat = persistence.listar(0, rowsPerPage);
        for (Categoria categoria : cat) {
            table.addRow(categoria);
        }
        currentPage = 1;
    ...
    public void controlLabels() {
        ...
        labelRegistros.setText(String.valueOf(cat.size()));
